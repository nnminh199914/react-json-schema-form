...
class MapSelection extends PureComponent {
  ...
  centerMap = () => {
    ...
    this.map
      .getView()
      .setCenter(
        proj.transform([centerLng, centerLat], "EPSG:4326", "EPSG:3857")
      );
  };
    ...
    return (
      <MapContainer expanded={mapExpanded}>
        {mapExpanded ? (
          ...
          <MapCover title="Expand map" onClick={this.handleMap} />
        )}
        <MapArea id={this.id} height={height} expanded={mapExpanded} />
        {mapExpanded ? (
          <MapClose title="Collpase map" onClick={this.handleMap} />
        ) : (
          ""
        )}
      </MapContainer>
